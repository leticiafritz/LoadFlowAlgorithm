...
class Gaussjacobi:
    ...
    def get_gaussjacobi_solution(self):
        print("\n")
            ...
            for k in range(len(code)):
                ...
                if code[k] == 2:
                    for m in range(len(code)):
                        if m != k:
                            sum_ye = sum_ye + self.y_matrix[k, m] * v_old[m]
                    v_current[k] = (1 / self.y_matrix[k, k]) * \
                                   (((p[k] - q[k] * 1j) / (v_old[k].real - v_old[k].imag * 1j)) - sum_ye)
        ...
        # Resultado:
        print("--------------------------------------------------")
        print("Processo convergiu com", iteration - 1, " iterações, pelo método de Gauss Jacobi.")
        print("Solução: ")
