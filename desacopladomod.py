...
class Desacopladomod:
    ...
    def get_jacobiana_matrix(self, code, p_calc_vector, q_calc_vector, v, teta):
        ...
        for k in range(len(code)):
            for m in range(len(code)):
                if k == m:
                    if code[k] == 1 or code[k] == 2:
                        h_matrix[row, col] = -(q_calc_vector[k] / v[k]) - (v[k] * self.b_matrix[k, k])
                    ...
                    if code[k] == 1 or code[k] == 2:
                        if code[m] == 1 or code[m] == 2:
                            if col >= row:
                                h_matrix[row, col] = v[m] * (self.g_matrix[k, m] * math.sin(teta[k] - teta[m]) -
                                                             self.b_matrix[k, m] * math.cos(teta[k] - teta[m]))
                                h_matrix[col, row] = -v[k] * (self.g_matrix[k, m] * math.sin(teta[k] - teta[m]) +
                                                              self.b_matrix[k, m] * math.cos(teta[k] - teta[m]))
                            if col < (h_matrix.shape[1] - 1):
                                col += 1
                            else:
        ...
        for k in range(len(code)):
            for m in range(len(code)):
                if k == m:
                    if code[k] == 2:
                        l_matrix[row, col] = ((v[k] ** (-2)) * q_calc_vector[k]) - self.b_matrix[k, m]
                    ...
                    if code[k] == 2:
                        if code[m] == 2:
                            if col >= row:
                                l_matrix[row, col] = (self.g_matrix[k, m] * math.sin(teta[k] - teta[m]) -
                                                      self.b_matrix[k, m] * math.cos(teta[k] - teta[m]))
                                l_matrix[col, row] = - (self.g_matrix[k, m] * math.sin(teta[k] - teta[m]) +
                                                        self.b_matrix[k, m] * math.cos(teta[k] - teta[m]))
                            if col < (l_matrix.shape[1] - 1):
                                col += 1
                            else:
    ...
    def get_desacopladomod_solution(self):
        print("\n")
        print("Iniciando o método de Newton Desacoplado Modificado ...")
        print("... As equações do fluxo de potência serão divididas por V[k] ...")
        ...
        while control_p > tolerance or control_q > tolerance:
            if control_p > tolerance:
                ...
                for k in range(len(code)):
                    if code[k] == 1 or code[k] == 2:
                        mismatches_p_vector[n] = mismatches_p_vector[n] / v[k]
                        n += 1
            ...
            if control_q > tolerance:
                ...
                for k in range(len(code)):
                    if code[k] == 2:
                        mismatches_q_vector[n] = mismatches_q_vector[n] / v[k]
                        n += 1
        ...
        print("Processo convergiu com", iteration_p - 1, " meia iteração ativa e ", iteration_q - 1,
              " meia iteração reativa, pelo método de Newton Desacoplado Modificado.")
        print("Solução: ")
